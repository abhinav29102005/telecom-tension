...
# ------------------------
# 1. Enhanced Synthetic Data Generator for High Accuracy
# ------------------------
def generate_synthetic_data(n_samples=5000):
    """
    Generate synthetic data with strong, high-signal churn patterns
    to enable high model accuracy.
    """
    np.random.seed(42)

    # Introduce a hidden 'customer profile' factor that strongly influences churn
    # Profile 0: High-Value, Low-Churn (Stable)
    # Profile 1: High-Risk, High-Churn (Volatile)
    # Profile 2: Medium-Risk, Mixed-Churn (Average)
    customer_profile = np.random.choice([0, 1, 2], n_samples, p=[0.4, 0.3, 0.3])
    
    # Generate features based on the customer profile
    ...
    # Stronger churn probability based on key features and the hidden profile
    churn_prob = (
        (customer_profile == 1).astype(int) * 0.9 +  # Profile 1 is highly likely to churn
        ...
        np.random.normal(0, 0.05, n_samples) # Reduced noise
    )
...
# ------------------------
# 4. Optimized Model Training with Stacking
# ------------------------
...
# ------------------------
# 5. Enhanced KMeans Clustering (remains the same)
# ------------------------
...
# ================================================================
# Streamlit UI (with minor updates for Stacking Classifier)
# ================================================================
...
with st.form("prediction_form"):
    ...
    with col1:
        ...
        multiple_lines = st.selectbox("Multiple Lines", [0, 1, 2], format_func=lambda x: ["No Phone", "Single Line", "Multiple Lines"][x])
...
# Model Insights and Business Intelligence
